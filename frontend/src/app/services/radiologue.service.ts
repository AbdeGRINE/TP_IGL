...
interface Bilan {
  nom: string;
  date_demande: string; // ISO date format (YYYY-MM-DD)
  date_recuperation: string | null;
  status: "En_cours" | "Terminé" | "Annulé"; // Assuming limited status values
  type: "Radiologique" | "Biologique" | "Autre"; // Adjust as needed
  redigant_laborantin: string | null;
  redigant_radiologue: string | null;
  consultation: number;
  graphique: "Non_Attaché" | "Attaché";
}
...
export class RadiologueService {
  ...
  //   return this.http
  //     .get<Bilan[]>(
  //       `${this.apiUrl}bilan/consulter_bilans_radiologiques_en_cours/`,
  //       { headers }
  //     )
  //     .pipe(
  //       tap((bilans) => {
